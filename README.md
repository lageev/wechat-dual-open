...
安装 [gum](https://github.com/charmbracelet/gum) 后，菜单支持方向键选择，确认框与输入体验更好；未安装时自动回退到数字菜单，不影响使用。
...
主界面顶部会常驻显示原版微信与所有双开实例的状态；选项 `3) 刷新状态` 仅重绘界面，不会重复打印一遍状态。
...
重新扫描并刷新主界面顶部的状态显示（版本号、Bundle ID、代码签名）。如果配置文件中记录了已不存在的应用，会标注「已删除」并建议清理。
